...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pysot is not None:
        ...
        model = get_backbone(cfg.BACKBONE.TYPE, **cfg.BACKBONE.KWARGS)
        load_pretrain(model, args.pysot)
    ...
    tb_writer = SummaryWriter(os.path.join(args.log, 'train_backbone_' + now))
    logger.info("tensorboard writer created @ " + str(os.path.join(args.log, now + '_log.txt')))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1 = validate(val_loader, model, criterion, args, tb_writer, epoch)
...
def train(train_loader, model, criterion, optimizer, epoch, args, tb_writer):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # logger.info("Time for computing output of 1 batch: {}".format(elapsed_time))
        # start_time = time.time()
        loss = criterion(output, target)
        # elapsed_time = time.time() - start_time
        # logger.info("Time for computing loss of 1 batch: {}".format(elapsed_time))
    ...
    tb_writer.add_scalar('acc@1/train', top1.avg, epoch)
    tb_writer.add_scalar('acc@5/train', top5.avg, epoch)
    tb_writer.add_scalar('loss/train', losses.avg, epoch)


def validate(val_loader, model, criterion, args, tb_writer, epoch):
    ...
    tb_writer.add_scalar('acc@1/val', top1.avg, epoch)
    tb_writer.add_scalar('acc@5/val', top5.avg, epoch)
